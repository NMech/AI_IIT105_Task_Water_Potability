...
###############################################################################
X        = pd.DataFrame(data[ColumnsUsed], columns=ColumnsUsed)
y        = data.Potability
...
y_train  = pd.Series(y_train, name="Potability")
...
if plotDiagrams == True:
    histPlotObj.HistPlot(XYconcat_train, "Potability", savePlot=[saveDiagrams, fig_filepath, "Features_Histogram"])  
#%%
###############################################################################
########################### Correlation Coefficients ##########################
###############################################################################
pearsonCorr  = XYconcat_train.corr(method="pearson")
spearmanCorr = XYconcat_train.corr(method="spearman")
if plotDiagrams == True:
...
###############################################################################
if plotDiagrams == True:
    yPred_rbf_svm_test   = rbf_svm.predict(X_test_prepared)                                             
    yScores_rbf_svm_test = rbf_svm.decision_function(X_test_prepared)
    clf_metricsPlot = ClassificationMetricsPlot(y_test)
